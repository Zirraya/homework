...
f11 x eps n
  ...
  where
    ...
    y1 = x^2
    y2 = x^3 / 2
    n' = min n 50  -- Ограничение максимальной длинны
    
    -- Реализация без цикал, оно не работает
    ys = y0 : y1 : y2 : [ nextValue i (ys !! (i-1)) (ys !! (i-2)) (ys !! (i-3)) | i <- [3..n'] ]
    
    nextValue i yn yn1 yn2 = (x * yn + yn2^2) / (fromIntegral i * yn1)

-- Упрощенная реализация zipWith4 
