...
В АВЛ-дереве высота гарантированно составляет O(log n), где n - количество
элементов в дереве. Операция вставки включает:

- Поиск места для вставки: O(log n)

- Вставка нового узла: O(1)  

- Балансировка (до 2 поворотов): O(1)

\textbf{Временная сложность одной операции вставки: O(log n)}

Для выполнения n операций вставки сложность составляет O(n log n).

 \section{Удаление в АВЛ дереве}

Операция удаления в АВЛ-дереве сохраняет балансировку и включает:
- Поиск удаляемого узла: O(log n)

- Поиск минимального в правом поддереве (при необходимости): O(log n)

- Балансировка (до 2 поворотов): O(1)

\textbf{Временная сложность одной операции удаления: O(log n)}

Для n операций удаления сложность составляет O(n log n).

 \section{Поиск в АВЛ дереве}

Поиск в АВЛ-дереве выполняется аналогично бинарному дереву поиска, но
благодаря балансировке гарантируется логарифмическая высота:
...
Поиск выполняется рекурсивным спуском от корня к листьям, на каждом
уровне отбрасывая половину оставшихся узлов
...
поиска и обходов в АВЛ-дереве составляет O(n logn) для n элементов.
...
Глубина стека вызовов зависит от высоты дерева и количества рекурсивных вызовов, что может быть O(log n) в худшем случае для операций вставки,удаления и поиска.
Таким образом, общий асимптотический анализ сложности расхода памяти составляет O(n+log n),
