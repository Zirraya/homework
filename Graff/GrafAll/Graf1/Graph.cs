    ...
    public class Graph
    {
        ...
        public Graph(string filename)
        {
            ...
            try
            {
                ...
                // Создаем все вершины
                foreach (int vertex in allVertices.OrderBy(v => v))
                {
                ...
                // Добавляем все рёбра без ограничений
                for (int i = 1; i < lines.Length; i++)
                {
                    string[] parts = lines[i].Split(':');
                    int vertex = int.Parse(parts[0].Trim());
                    ...
                    if (!string.IsNullOrWhiteSpace(parts[1]))
                    {
                        ...
                        foreach (string edgeStr in edges)
                        {
                            ...
                            // Убрано условие vertex <= target и suppressReverse
                            try
                            {
        ...
        // Добавление ребра/дуги
        public void AddEdge(int from, int to, double weight = 1.0, bool suppressReverse = false)
        {
            ...
            if (!IsDirected && from != to && !suppressReverse)
            {
                // Не проверяем существование обратного ребра, чтобы избежать рекурсии
